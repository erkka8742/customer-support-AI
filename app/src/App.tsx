...
function InteractiveElement({ type, isActive }: { type: Slide['interactiveType']; isActive: boolean }) {
  ...
  const solveManualTicket = async (ticketId: number) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/manual-review-tickets/${ticketId}/solve`, {
        method: 'POST',
  ...
  const fetchTicketAnswer = async (ticketId: number) => {
    ...
    setExpandedSources([])
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/tickets/${ticketId}/answer`)
      const data = await response.json()
  ...
  const deleteTicket = async (ticketId: number) => {
    await fetch(`${import.meta.env.VITE_API_URL}/api/tickets/${ticketId}`, { method: 'DELETE' })
    fetchTickets()
  }
  ...
  const resolveTicket = async (solved: boolean) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_API_URL}/api/tickets/${selectedTicketAnswer.ticket.id}/resolve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
